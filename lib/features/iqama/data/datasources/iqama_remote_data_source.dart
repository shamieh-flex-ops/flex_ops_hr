...
import 'package:flex_ops_hr/features/iqama/data/models/create_iqama_renewal_response_model.dart';
...
abstract class IqamaRemoteDataSource {
  ...
  Future<CreateIqamaRenewalResponseModel> createIqamaRenewal(CreateIqamaRenewalParams params);
}
...
class IqamaRemoteDataSourceImpl implements IqamaRemoteDataSource {
  ...
  @override
  Future<CreateIqamaRenewalResponseModel> createIqamaRenewal(CreateIqamaRenewalParams params) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final token = prefs.getString('token');

      final response = await dio.post(
        ApiConstance.createIqamaRenewal,
        data: {
          "new_iqama_id": params.newIqamaId,
      ...
      return CreateIqamaRenewalResponseModel.fromJson(response.data);
    } catch (e) {
      throw _handleError(e);
    }
  }
